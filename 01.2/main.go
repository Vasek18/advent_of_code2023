...
func getNumberFromString(input string) int {
	...
	for charPosition, char := range input {
		if unicode.IsDigit(char) {
			if firstNumber == 0 {
				firstNumber, _ = strconv.Atoi(fmt.Sprintf("%c", char))
			}

			lastNumber, _ = strconv.Atoi(fmt.Sprintf("%c", char))
		} else {
			leftoverOfString := input[charPosition:]
			for numberInText, number := range numbersInText {
				if strings.Index(leftoverOfString, numberInText) == 0 {
					if firstNumber == 0 {
						firstNumber = number
					}
